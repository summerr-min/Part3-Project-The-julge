...
| **송민하** | **알바님(구직자) 페이지** (프로필 등록/관리, 신청 내역 및 알림 시스템, 공통:DropDown)    |
| **이충만** | **공고 리스트 페이지** (상세 필터링, 정렬 시스템, 공통: footer)                          |
| **김용현** | **사장님(구인자) 페이지** (가게 정보 등록/수정(이미지업로드), 공고 리스트(지난공고/마감완료, 무한스크롤), 공고 등록/수정(timepicker), 지원자 리스트(지원자 승인/거절/취소), 공통 레이아웃 설계) |
